...
└── shared/                # Auth hook, WASM loader, page helpers
```

Pages are rendered on the server (Maud + htmx). `build.rs` bundles and minifies
the scripts and styles beside the templates into the binary, served at
content-hashed `/assets/` URLs; only the wasm-pack output is read from
`crates/public_ui/pkg` (`[ui_settings].ui_dir`). See
`crates/coordinator/src/templates/README.md`.
