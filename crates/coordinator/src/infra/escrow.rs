...
/// Keep LND lease ownership until the signed transaction is durably stored.
...
pub async fn generate_escrow_tx(
    ...
    amount_sats: u64,
    lease_deadline: u64,
) -> Result<PreparedEscrow, anyhow::Error> {
...
/// Recover the exact script of an escrow created before or after the reclaim
/// branch was introduced. The output commits to the version; callers cannot
/// substitute keys or policy while preserving this check.
pub fn escrow_descriptor_for_output(
    coordinator_pubkey: &PublicKey,
    user_pubkey: &PublicKey,
    payment_hash: &[u8; 32],
    output: &TxOut,
) -> Result<Descriptor<PublicKey>, anyhow::Error> {
    let current = create_escrow_descriptor(coordinator_pubkey, user_pubkey, payment_hash)?;
    if current.script_pubkey() == output.script_pubkey {
        return Ok(current);
    }
    let legacy = Descriptor::from_str(&format!(
        "wsh(or_d(multi(2,{coordinator_pubkey},{user_pubkey}),and_v(v:pk({user_pubkey}),and_v(v:sha256({}),older(144)))))",
        hex::encode(payment_hash),
    ))?;
    if legacy.script_pubkey() == output.script_pubkey {
        return Ok(legacy);
    }
    Err(anyhow!(
        "Escrow output does not match its registered keys and payment hash"
    ))
}
...
mod tests {
    ...
    #[test]
    fn stored_escrows_preserve_their_original_descriptor() {
        let secp = Secp256k1::new();
        let (_, coordinator) = key(&secp, 1);
        let (_, user) = key(&secp, 2);
        let payment_hash = [3; 32];
        let current = create_escrow_descriptor(&coordinator, &user, &payment_hash).unwrap();
        let legacy: Descriptor<PublicKey> = Descriptor::from_str(&format!(
            "wsh(or_d(multi(2,{coordinator},{user}),and_v(v:pk({user}),and_v(v:sha256({}),older(144)))))",
            hex::encode(payment_hash),
        )).unwrap();
        assert_ne!(current.script_pubkey(), legacy.script_pubkey());
        for expected in [current, legacy] {
            ...
            let selected =
                escrow_descriptor_for_output(&coordinator, &user, &payment_hash, &output).unwrap();
            assert_eq!(
                selected.explicit_script().unwrap(),
                expected.explicit_script().unwrap()
            );
            assert!(escrow_descriptor_for_output(&coordinator, &user, &[4; 32], &output).is_err());
            assert!(
                escrow_descriptor_for_output(&user, &coordinator, &payment_hash, &output).is_err()
            );
        }
    }
