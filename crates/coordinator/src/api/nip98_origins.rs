...
mod tests {
    ...
    fn origins_must_be_http_urls() {
        ...
        for origin in [
            "https://user@weather.example",
            "https://weather.example/path",
            "https://weather.example/#fragment",
            "https://weather.example/?query",
        ] {
            assert!(Nip98Origins::new([origin]).is_err());
        }
        assert!(!accepts("https://user@weather.example/api", "/api"));
        assert!(!accepts("https://weather.example/api#fragment", "/api"));
    }
}
