...
impl Metric {
    ...
    /// Readings are Fahrenheit and knots.
    pub fn unit(self) -> &'static str {
        match self {
            Metric::TempHigh | Metric::TempLow => "°F",
            Metric::WindSpeed => "kt",
        }
    }
...
pub enum PickState {
    ...
    Out,
    /// The window has closed; the observation is final.
    Final,
}

/// How `pick` stands against the observation so far. `window_closed` means every observation
/// the oracle will score has been made.
pub fn pick_state(
    pick: &ValueOptions,
    metric: Metric,
    forecast: Option<f64>,
...
pub struct Standing {
    ...
    pub rank: usize,
    /// Another entry has the same score.
    pub tied: bool,
    /// Points.
    pub score: u64,
}
...
pub fn standings(scores: &[u64]) -> Vec<Standing> {
    scores
        ...
        .map(|&score| Standing {
            ...
            tied: scores.iter().filter(|&&other| other == score).count() > 1,
            score,
        })
        .collect()
}
...
/// Percent of the pool each paid place receives (the coordinator's contract weights).
pub fn place_weights(places: usize) -> Vec<u64> {
    match places {
        1 => vec![100],
        2 => vec![70, 30],
        3 => vec![45, 35, 20],
...
/// What each entry would be paid, in sats, for `scores` (index-aligned with the returned list).
///
/// `entries` pairs each score with the oracle's entry id; `ticket_order` gives each entry's
/// position among the contract's players. When nobody scores, or there are fewer entries than
/// paid places, every entry is refunded an equal share, the remainder going to the first players.
pub fn prizes(
    entries: &[(u64, uuid::Uuid)],
    ticket_order: &[usize],
    places: usize,
    pool_sats: u64,
) -> Vec<u64> {
    let count = entries.len();
    let mut paid = vec![0; count];
    if count == 0 {
        return paid;
    }
    if count < places || entries.iter().all(|(score, _)| *score == 0) {
        let share = 100 / count as u64;
        let remainder = 100 % count as u64;
        for (index, prize) in paid.iter_mut().enumerate() {
            let weight = share + u64::from((ticket_order[index] as u64) < remainder);
            *prize = pool_sats * weight / 100;
        }
        return paid;
    }
    let weights = place_weights(places);
    for (place, index) in payout_order(entries).into_iter().take(places).enumerate() {
        paid[index] = pool_sats * weights.get(place).copied().unwrap_or(0) / 100;
    }
    paid
}
...
mod tests {
    ...
    fn points_match_the_oracle() {
        ...
        for missing in [
            ...
            for pick in [Over, Par, Under] {
                ...
                assert_eq!(
                    pick_state(&pick, Metric::TempHigh, missing.0, missing.1, false),
                    PickState::Pending
                );
            }
        }
        ...
        for (metric, cases) in [
            ...
            for (observed, expected) in cases {
                for (pick, expected) in [Over, Par, Under].iter().zip(expected) {
                    assert_eq!(
                        pick_state(pick, metric, forecast, Some(observed), false),
                        expected,
                        "{metric:?} {pick:?} with {observed} observed against 70"
                    );
                    assert_eq!(
                        pick_state(pick, metric, forecast, Some(observed), true),
                        PickState::Final
                    );
                }
            }
    ...
    fn temperature_states_change_at_the_rounding_boundary() {
        use PickState::{LockedIn, OffTrack, OnTrack, Out};
        let state =
            |pick, metric, observed| pick_state(pick, metric, Some(70.0), Some(observed), false);
        ...
        assert_eq!(
            pick_state(&Par, Metric::WindSpeed, Some(10.0), Some(10.0), false),
            OnTrack
        );
        assert_eq!(
            pick_state(&Over, Metric::WindSpeed, Some(10.0), Some(10.4), false),
            LockedIn
        );
    }
        ...
        for metric in Metric::ALL {
            for observed in [68.0, 69.6, 70.0, 70.3, 70.5, 72.0] {
                for pick in [Over, Par, Under] {
                    ...
                    let state = pick_state(&pick, metric, Some(70.0), Some(observed), false);
    ...
    fn equal_scores_share_a_rank() {
        let ranked = standings(&[30, 50, 30, 10, 50]);
        let ranks: Vec<(usize, bool)> = ranked.iter().map(|s| (s.rank, s.tied)).collect();
        assert_eq!(
            ranks,
            vec![(3, true), (1, true), (3, true), (5, false), (1, true)]
        );
        assert!(standings(&[]).is_empty());
    }

    #[test]
    fn prizes_follow_the_oracle_order_and_refund_all_when_nobody_scores() {
        let early = Uuid::from_u128(1);
        let late = Uuid::from_u128(2);
        let third = Uuid::from_u128(3);
        // A tie goes to the earlier entry id; the rest follow by score.
        let entries = [(20, late), (20, early), (40, third)];
        assert_eq!(payout_order(&entries), vec![2, 1, 0]);
        assert_eq!(
            prizes(&entries, &[0, 1, 2], 2, 10_000),
            vec![0, 3_000, 7_000]
        );
        // Nobody scored: everyone is refunded, the remainder to the first players.
        let nobody = [(0, early), (0, late), (0, third)];
        assert_eq!(
            prizes(&nobody, &[2, 0, 1], 1, 9_000),
            vec![2_970, 3_060, 2_970]
        );
        assert!(prizes(&[], &[], 1, 1_000).is_empty());
    }
}
