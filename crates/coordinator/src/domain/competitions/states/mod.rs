...
mod tests {
    ...
    #[test]
    fn the_first_entry_moves_a_competition_to_a_state_that_reloads() {
        let mut competition = competition();
        competition.total_entries = 1;
        let CompetitionStatus::Created(created) =
            CompetitionStatus::Created(Created::from_competition(competition.clone()))
        else {
            unreachable!()
        };
        let next = created.first_entry_added();
        assert_eq!(next.state_name(), "collecting_entries");
        assert_eq!(stored(next), "collecting_entries");
        assert_eq!(
            CompetitionStatus::from(competition).state_name(),
            "collecting_entries",
