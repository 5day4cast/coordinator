class WeatherData {
  ...
  async get_competition_last_forecast(competition) {
    ...
    const forecasts = await this.get_forecasts(competition.locations, {
  ...
  async get_forecasts(station_ids, time_range) {
    ...
    let response = await fetch(
      `${this.base_url}/stations/forecasts?start=${time_range.start}&end=${time_range.end}&station_ids=${stations}`,
    );
    if (!response.ok) {
      throw new Error(`Failed to get stations, status: ${response.status}`);
    }
  ...
  async get_observations(station_ids, time_range) {
    let stations = station_ids.join(",");
    let response = await fetch(
      `${this.base_url}/stations/observations?start=${time_range.start}&end=${time_range.end}&station_ids=${stations}`,
    );
    if (!response.ok) {
      throw new Error(`Failed to get stations, status: ${response.status}`);
    }
