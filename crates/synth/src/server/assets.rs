...
mod tests {
    ...
    /// htmx 4 and its SSE extension come first and whole, then synth's own script, minified.
    #[test]
    fn the_script_carries_htmx_its_sse_extension_and_the_copy_button() {
        ...
        assert!(
            js.contains("this.version=\"4.0.0\""),
            "htmx 4.0.0, the vendored release"
        );
        let sse = js
            .find("registerExtension(\"sse\"")
